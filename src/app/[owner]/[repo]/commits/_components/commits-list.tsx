...
function CommitsListInner({ owner, repo, branch, config }: CommitsListProps) {
  ...
  const { data, isLoading, isError, error } = api.commits.listCommits.useQuery(
    {
        ...
        <div className="flex items-center gap-2">
          {author && (
            <span className="inline-flex items-center gap-1 rounded-md bg-surface-tertiary px-2 py-1 text-sm">
              ...
              <button
                type="button"
                onClick={() => handleAuthorToggle("author", `author:${author}`)}
                className="ml-0.5 rounded p-0.5 text-text-muted hover:text-text-primary transition-colors"
        ...
        <div>
          <div className="space-y-6">
            {groupedCommits.map(([dateLabel, commits]) => (
              <div key={dateLabel}>
                <h3 className="mb-2 px-2 py-1 text-sm font-medium text-text-secondary">
                  {dateLabel}
                </h3>
          ...
          <div className="mt-6 flex items-center justify-center gap-4">
            ...
            <button
              ...
              className={cn(
                btnBase,
                nav.page >= totalPages ? btnDisabled : btnActive,
              )}
            >
              Next
