...
type AssigneeOperation = { id: number, op: 'add' | 'remove', assignee: Assignee };
...
export function AssigneeSection({
	...
	const { data: repoAssignees } = api.pulls.listAssignees.useQuery({ owner, repo });
	...
	const handleAdd = (assignee: Assignee) => {
		...
		addMutation.mutate({ owner, repo, number, assignee: assignee.login }, {
			onError: () => {
	...
	const handleRemove = (assignee: Assignee) => {
		...
		removeMutation.mutate({ owner, repo, number, assignee: assignee.login }, {
			onError: () => {
				setOperations(prev => prev.filter(op => op.id !== id))
			}
		});
...
function AssigneeSectionSettings({ repoAssignees, assignees, operations, onAddAssignee, onRemoveAssignee }: {
	...
	const filteredAssignees = (sortedAssignees.current ?? repoAssignees ?? []).filter(
		(a) => a.login.toLowerCase().includes(search.toLowerCase()),
	);
						...
						{filteredAssignees.length === 0 ? (
							...
							filteredAssignees.map((assignee) => {
								...
								return (
									...
									>
										<img src={assignee.avatar_url} alt="" className="h-5 w-5 shrink-0 rounded-full" />
	...
	return (
		<ul className="space-y-2">
			{displayAssignees.map((assignee) => (
				<li className="group flex items-center gap-2 text-sm" key={assignee.login}>
...
function applyOperations(assignees: Assignee[], operations: AssigneeOperation[]): Assignee[] {
	let updatedAssignees = [...assignees];

	for (const op of operations) {
		if (op.op === 'add' && !updatedAssignees.some(a => a.login === op.assignee.login)) {
			updatedAssignees.push(op.assignee);
		}
		if (op.op === 'remove') {
			updatedAssignees = updatedAssignees.filter(a => a.login !== op.assignee.login)
		}
	}
